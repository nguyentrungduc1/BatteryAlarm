...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupUI() {
        ...
        binding.etDndStart.setText(prefs.getInt("dnd_start_hour", 22).toString())
        binding.etDndEnd.setText(prefs.getInt("dnd_end_hour", 6).toString())

        binding.switchDnd.setOnCheckedChangeListener { _, isChecked ->
            saveDndSettings(isChecked)
        }
        ...
        binding.btnSave.setOnClickListener {
            ...
            // Also save DND settings when pressing Save
            saveDndSettings(binding.switchDnd.isChecked)
    ...
    private fun saveDndSettings(enabled: Boolean) {
        val startStr = binding.etDndStart.text.toString()
        val endStr = binding.etDndEnd.text.toString()
        val start = startStr.toIntOrNull()
        val end = endStr.toIntOrNull()

        if (start == null || start < 0 || start > 23) {
            Toast.makeText(this, "Giờ bắt đầu không hợp lệ (0–23)", Toast.LENGTH_SHORT).show()
            binding.switchDnd.isChecked = false
            return
        }
        if (end == null || end < 0 || end > 23) {
            Toast.makeText(this, "Giờ kết thúc không hợp lệ (0–23)", Toast.LENGTH_SHORT).show()
            binding.switchDnd.isChecked = false
            return
        }
        ...
        prefs.edit()
            ...
            .putInt("dnd_start_hour", start)
            .putInt("dnd_end_hour", end)
            .apply()

        if (enabled) {
            Toast.makeText(this, "Không làm phiền: ${start}h – ${end}h", Toast.LENGTH_SHORT).show()
        }
        updateService()
    }
